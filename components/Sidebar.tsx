...
export function Sidebar({ className }: SidebarProps) {
  ...
  const groupHistory = (items: FormSchema[]) => {
    ...
    return [
      ...
      { label: 'Yesterday', items: items.filter(f => f.createdAt >= yesterday && f.createdAt < today) },
      { label: 'Previous 7 Days', items: items.filter(f => f.createdAt >= lastWeek && f.createdAt < yesterday) },
  ...
  return (
    <aside className={cn("w-72 border-r border-zinc-900/50 flex flex-col bg-[#09090b] h-full", className)}>
      {/* Header / New Form */}
      <div className="p-6">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-sm tracking-tight text-zinc-200">Monk Form</h2>
          </div>
          <Button 
            variant="ghost" 
            size="icon" 
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-6 custom-scrollbar pb-4">
        {history.length === 0 ? (
          <div className="px-3 py-10 text-center">
            ...
            <p className="text-[10px] text-zinc-600 uppercase tracking-widest">Empty</p>
          </div>
        ) : (
          groupedHistory.map((group) => (
            <div key={group.label} className="space-y-1">
              <div className="px-3 py-2 text-[10px] font-medium text-zinc-500 uppercase tracking-wider">{group.label}</div>
              <div className="space-y-0.5">
                {group.items.map((form) => (
                  ...
                  >
                    <span className="text-xs font-medium truncate pr-6">{form.title}</span>
                    <Button
                      variant="ghost"
                      size="icon"
      ...
      <div className="p-4 mt-auto border-t border-zinc-900/50">
        <Link href="/settings">
          <Button variant="ghost" className="w-full justify-start gap-3 rounded-md py-6 hover:bg-zinc-900 text-zinc-400 hover:text-zinc-100 group transition-all">
            ...
            <div className="flex flex-col items-start text-left">
              ...
              <span className="text-[9px] text-zinc-600 uppercase tracking-tighter">Configurations</span>
            </div>
          </Button>
        </Link>
      </div>
